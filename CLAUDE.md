...
- **Bold needs a concrete font family.** The default UI font (`get_default_font_family()` → `.AppleSystemUIFont` on macOS) resolves heavy weights poorly, and GPUI does **not** synthesize (fake) bold — so `.font_weight(FontWeight::BOLD)` alone often looks unchanged. Render the text in a real family via `.font_family(get_font_family())` on the element (or on an ancestor — font-family cascades, which is why the key-tree badge looks bold: its `ListItem` sets the family for the whole row). Mind the family's actual faces: Menlo (macOS mono) only ships Regular/Bold, so `EXTRA_BOLD`/`BLACK` collapse to `Bold`.
