...
### Development setup
Zedis is built with Rust (edition 2024; **Rust 1.98.1** is the toolchain we build with — pinned in `rust-toolchain.toml`, which rustup applies automatically) and [GPUI](https://www.gpui.rs/).
...
### 开发环境
Zedis 使用 Rust(edition 2024;我们以 **Rust 1.98.1** 构建,版本固定在 `rust-toolchain.toml` 中,rustup 会自动应用)与 [GPUI](https://www.gpui.rs/) 开发。
