...
完整 **TLS/SSL**（自定义 CA、客户端证书）与 **SSH 隧道**（密码、私钥、Agent）。把连接整理进可命名、可折叠的**分组**并重排。任意勾选连接导出为 JSON（默认剥离凭据）—— 或设置口令，生成紧凑、可跨机的**加密分享码**（`ZEDIS1.…`，Argon2id + AES-256-GCM），只有该口令能解开；导入对话框会识别分享码并提示输入口令。也可粘贴 `redis://` 连接串或 **Redis Insight** 的数据库导出一键迁入 —— 多个数据库一次到位。
