...
Zedis 启动时会检查 GitHub 上是否有新版本 —— 最多每两天一次，可按版本跳过，也可在设置里彻底关掉（标题栏另有手动的**检查更新**）。发现新版本时标题栏出现更新 chip；下载时会先用发布清单里的 **SHA-256** 校验安装包，再交给系统安装程序打开 —— 绝不在后台替换正在运行的二进制。进度同时显示在对话框和 chip 的百分比上。
