...
标签、备注、收藏与搜索历史存于**本地 redb 文件** —— 完全不占 Redis 存储，也不发往任何地方。连接密钥**加密存储**，以 JSON 分享连接时**默认剥离凭据**。可选的 AI 分析只发送 key 的**名称、大小与 TTL —— 绝不含 value**，且只发往**你自己配置**的 OpenAI 兼容接口。自定义脚本查看器经你本机的 Shell 本地运行。**无遥测、无账号、无云端。**Zedis 唯一主动发起的请求是**启动时的更新检查**——最多每两天一次，只带应用版本号，发往 GitHub，GitHub 不可达时改发 Gitee 镜像，且可在设置中关闭（见下文*保持更新*）。Windows 版二进制经 SignPath Foundation 做了 **Authenticode 签名**，因此你运行的安装包可被验证为 GitHub Actions 从本仓库构建的那一个。
...
### 账号与归属
**必须登录；条目在你共享之前都是私有的；Redis 密码永远不会下发到浏览器。**

`ZEDIS_BRIDGE_USERS`（`name@password,name2@password2`）是必填项 —— 没有它 bridge 拒绝启动。页面要求输入用户名与密码；脚本可用 HTTP Basic 走同一套账号。**服务器条目归添加它的账号所有**，其他人看不到，除非勾选 **Shared** —— 勾选后它就是所有人的。没有角色划分：任何账号都可以编辑或删除共享条目。Redis 凭据只留在 bridge 上；浏览器拿到的是占位符而非密文，只改设置的编辑会保留已存的密码。明文 http 下账号密码是裸奔的，所以试用之外的场景都该放在 HTTPS 反向代理之后 —— 如果 bridge 对内网之外可达，再加一层限流。

### 浏览器里没有什么
**凡是"一问一答"的都能用；流式面板和需要落盘的功能留在桌面端。**

键树、所有值编辑器、终端、指标、慢查询、CONFIG、客户端、内存分析与值搜索都照常可用。不提供：流式面板（`MONITOR`、Pub/Sub、键空间通知）、拓扑与 Sentinel 管理、Lua 脚本库与 Protobuf schema 编辑器、跨库键搜索、迁移（文件导入 / 导出）与跨服务器对比、连接诊断、回收站以及 1 小时 / 24 小时 / 7 天的指标历史 —— 后两者需要能挺过刷新的存储 —— 还有被浏览器自己占用的快捷键（⌘N / ⌘T / ⌘W）。标签、备注、收藏与已保存脚本可以用，但只存在于当前页面：刷新即清空，首次连接时会有一条提示说明这一点。被裁掉的面板会渲染与"服务端缺少该命令"相同的说明性占位，而不是直接报错。停在后台标签页的页面会自行放慢轮询。桌面端仍是功能完整的客户端。
...
**⌘K** 模糊搜索服务器、各面板与当前连接已加载的键（方向键移动、Enter 执行、Esc 关闭）。**⌘P** 是当前连接"最近打开的键"的快速打开面板 —— 即 Zed / VS Code 的那个手势，在工具页里同样可用。**⌘/** 打开只读的分组浮层，按平台符号列出所有快捷键：⌘N 新建键、⌘S 保存、⌘R 重载键树、⌘⇧R 重载当前值、⌘T 设置 TTL、⌘⌫ 删除、⌘E 重命名、⌘F 过滤、⌘J 终端、⌘1–8 切换工作区标签页、Esc 返回。**⌘+ / ⌘− / ⌘0** 缩放整个界面（与设置里字号滑块是同一个值）。所有快捷键都可以**自定义**：设置 → *键盘快捷键* 会打开配置目录下的 `keybindings.toml`，每个动作一行 `id = "按键"`，重启后速查浮层显示的就是你的按键。
...
Zedis 启动时会检查 GitHub 上是否有新版本 —— 最多每两天一次，可按版本跳过，也可在设置里彻底关掉（标题栏另有手动的**检查更新**）。GitHub 的发布 CDN 在中国大陆很慢甚至不可达，因此检查、清单、更新说明与下载都会回落到 **[Gitee 镜像](https://gitee.com/vicanso/zedis)** —— 它从同一个 release 逐个文件复制而来。发现新版本时标题栏出现更新 chip；无论由哪一方提供字节，下载时都会先用发布清单里的 **SHA-256** 校验安装包。**macOS 上随后原地完成安装**：静默挂载 DMG、校验 bundle 标识、把新版覆盖到旧版位置（旧包移入临时目录暂存，绝不在进程运行期间删除），**一键 Restart** 即重启进新版本；任何阻碍原地安装的情况都会回落到经典的拖拽安装窗口。Windows 交给 MSI 安装程序、Linux 交给桌面处理器打开。进度同时显示在对话框和 chip 的百分比上。

设置里的**包含预发布版本**开关会把预发布版和滚动构建的 **nightly** 也纳入检查（nightly 构建默认开启，并按构建时间比较，新的 nightly 一发布就会提示）。
...
Zedis **每个配置目录只运行一个实例**：再次启动只会把已开的窗口带到前台；在浏览器或文档里点击 **`redis://` / `rediss://` 链接**、或在命令行传入链接，都会在这个实例里打开——落到 host / port / TLS / 用户名相同的已保存连接上（没有就把链接存为新连接），并进入链接里的 `/db`。协议由 macOS bundle、Linux 桌面文件和 Windows 安装包注册；macOS 上若被其他客户端抢占，设置里有按钮重新接管。macOS 还有标准的 **Edit** 与 **Window** 菜单，**PROD 标签的连接会给标题栏染色**，最大化的窗口下次启动仍是最大化。设置 → *日期与时间* 可选择全应用时间戳使用的**时区**（本地或 UTC）与**日期格式**；*本地数据* 可把标签、备注、收藏、脚本查看器、Lua 脚本与 proto 绑定**导出**为一个 JSON 文件，也能**导入**这样的文件并合并到本机。
