...
按版本过滤的命令补全与内联参数 / 说明提示，每条建议都链到 redis.io 上对应的文档页。命令历史**按服务器**分别保存并持久化到本地：↑/↓ 逐条回溯，**Ctrl+R** 打开增量式反向搜索。粘贴的命令即使还带着开头的 `redis-cli` 也能照常执行；阻塞类命令（`BLPOP` / `BRPOP` / `BZPOPMIN`、`XREAD … BLOCK`、`WAIT` 等）会被提前拒绝，以免占住共享连接。一键 **Batch** 模式把 REPL 换成多行编辑器——每行一条命令，用 `⌘`/`Ctrl`+`Enter` 运行（危险命令仍走确认对话框）。输入 **`? <问题>`** 即可用自然语言向已配置的 AI 端点询问对应命令——建议填入输入框供审阅（绝不自动执行，危险确认与只读锁照常生效），并附一句界面语言的解释。只发送问题和服务器*元数据*（版本、部署形态、模块），绝不发送 key 的值。
