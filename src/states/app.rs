...
// The route types and the server-list operations are their own files; both
// are still `states::app` to every caller (`pub use` below, and the methods
// are `ZedisAppState`'s as before).
mod route;
mod servers;
pub use route::{Route, ServerView};
