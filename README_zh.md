...
- **丰富内容解码**：
  ...
  - **Unix 时间戳**：恰好 10 位（秒）或 13 位（毫秒）的 epoch 字符串会被自动识别，预览为本地时间 + UTC 日期——原始值保持不变且可编辑。
...
- **内存分析器**：可视化排查 **BigKey**，优化存储效率，预防 OOM。Top-N 表支持按 **大小 / 最热 / 最冷** 排序——根据服务端 `maxmemory-policy` 自动选用 `OBJECT FREQ`（LFU）或 `OBJECT IDLETIME`（LRU），一键定位真正值得缓存（或淘汰）的 key。同一次 SCAN 还会在同一视图中（无需切换 Tab）同步展示 **TTL 分布直方图**（`<1m / <1h / <1d / <7d / ≥7d / 无 TTL`）——一眼识别"凌晨 3 点集中淘汰"陷阱、查看 `PERSIST`（内存泄漏红旗）占比，并在 ratio<1 采样时直接读到估算的全量键数。点击 **AI 分析** 即可将当前报告（前缀分组、最大 key、TTL 分布、淘汰策略）转成 Markdown 摘要，提交到任意 **OpenAI 兼容** 接口（在设置中填写 Base URL 与 API Key，密钥加密存储），把可执行的优化建议内联渲染出来——只发送 key 的*名称*、大小与 TTL，绝不发送 value。兼容 OpenAI、Claude（通过 Anthropic 的 OpenAI 兼容端点 `https://api.anthropic.com/v1/`）以及其它任意 OpenAI 兼容服务；建议会用应用当前界面语言返回。
...
- **集成 CLI 与 Workbench**：内置 `redis-cli` 终端，带按版本过滤的命令补全和内联参数/说明提示。一键切换 **Batch** 模式可把单行 REPL 换成多行编辑器——每行写一条命令，用 `⌘`/`Ctrl`+`Enter` 一次运行整段脚本（危险命令仍走确认对话框）。
