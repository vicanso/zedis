...
| 🔑 **Keys & 数据** | 带 TTL chip 的命名空间树 · 标签 / 备注 / 收藏 · 重命名 · 字段级 TTL · 版本历史 · 本地回收站(24h)· 文件导入导出 · 批量操作(Tools 导出、前缀过滤、二进制 / JSON / CSV)· 跨服务器复制 & 对比 |
...
| ⌨️ **效率** | 多连接工作区标签页 · ⌘K 面板 · ⌘⇧F 多数据库键搜索 · ⌘/ 快捷键速查 · 带补全的 redis-cli · AI 命令助手（终端内 `?`）· 多行 Batch 模式 · Lua 脚本库 · 可关闭的更新检查(带下载进度)· 滚动文件日志 · 导出诊断包（日志、崩溃报告、脱敏配置、连接状态打成一个 zip） |
