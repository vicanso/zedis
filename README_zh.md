...
- 🌐 **连接一切** —— TLS/SSL、SSH 隧道（含带口令的加密密钥）、Cluster/Sentinel、Redis Insight 导入，以及 8 种界面语言。
...
| 🔑 **Keys & 数据** | 带 TTL chip 的命名空间树 · 标签 / 备注 / 收藏 · 重命名 · 字段级 TTL · 版本历史 · 本地回收站(24h)· 文件导入导出 · 批量操作(含 JSON/CSV 导出)· 跨服务器复制 & 对比 |
| 🔐 **安全 & 隐私** | 环境标签 + PROD 升级确认 · 只读锁 · ACL 编辑 · TLS/SSL & SSH · 分阶段连接诊断 · 每机密钥加密 · 纯本地、无遥测 |
| ⌨️ **效率** | 多连接工作区标签页 · ⌘K 面板 · ⌘⇧F 多数据库键搜索 · ⌘/ 快捷键速查 · 带补全的 redis-cli · AI 命令助手（终端内 `?`）· 多行 Batch 模式 · Lua 脚本库 · 可关闭的更新检查(带下载进度)· 滚动文件日志 |
