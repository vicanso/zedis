...
- **Time Series Viewer** (RedisTimeSeries module): Selecting a `TSDB-TYPE` key opens a dedicated chart — `TS.INFO` surfaces total samples / memory / retention / chunk count and labels, while `TS.RANGE` (server-side `AVG` aggregation, bucketed to ~240 points so even a multi-million-sample series stays responsive) feeds a GPU-rendered line chart with `15m / 1h / 6h / 24h / 7d / All` range toggles. No separate module gate is needed — a key only resolves to this type when the `timeseries` module is loaded, so the viewer self-gates by the key's existence.
- **Probabilistic Structures** (RedisBloom module): Bloom filter, Cuckoo filter, Count-Min Sketch, Top-K, and t-digest keys — which used to render as opaque binary — now open a dedicated read-only viewer. Each renders its `*.INFO` stats (capacity, size, error rate, ...); Top-K also lists its current heavy hitters (`TOPK.LIST … WITHCOUNT`) and t-digest adds min / max / p50 / p90 / p99 (`TDIGEST.QUANTILE`). Dispatched by the key's module TYPE, so it appears only where RedisBloom is loaded.
- **Vector Set + KNN** (Redis 8): Native `vectorset` keys (previously unviewable) open a dedicated viewer — `VINFO` / `VCARD` / `VDIM` metadata, a `VRANDMEMBER` element sample, and an **interactive KNN search**: type (or click) an element to run `VSIM … WITHSCORES` and see its ranked nearest neighbours with similarity scores. Clicking a neighbour re-searches from it, so you can walk the HNSW graph hop by hop. Read-only.
...
- **Integrated CLI & Workbench**: A built-in terminal for `redis-cli` with version-aware command completion and inline argument/summary hints. A one-click **Batch** mode swaps the single-line REPL for a multi-line editor — write one command per line and run the whole script at once with `⌘`/`Ctrl`+`Enter` (dangerous lines still route through the confirm dialog).
