...
- **Memory Analyzer**: Visually hunt down **BigKeys** and optimize storage efficiency to prevent OOMs. Sort the Top-N table by **Size / Hottest / Coldest** — `OBJECT FREQ` or `OBJECT IDLETIME` is auto-selected based on the server's `maxmemory-policy`, so you can find the keys actually worth caching (or evicting) in a single click. The same SCAN run also feeds a **TTL distribution histogram** (`<1m / <1h / <1d / <7d / ≥7d / No TTL`) on a sibling tab — spot the "3 AM expiry cliff" pattern at a glance, see what share of keys is `PERSIST` (the memory-leak red flag), and read the estimated cluster-wide count even when sampling with `ratio < 1.0`.
...
- **Key Tags & Notes** *(client-side only)*: Annotate any key with a colour tag (red / orange / yellow / green / blue / purple) and a free-form private note — everything lives in the local redb file, **zero Redis storage cost**, never leaves the machine. Tagged rows in the key tree carry a 4 px colour bar on the left edge, and hovering reveals the note as a tooltip. Each tag colour can be used as a one-click filter on the tree (the toolbar dropdown only shows up once you've actually tagged something). The editor toolbar gets a swatch button next to the favourite star — same dialog opens whether you launch it from there or from the tree's right-click "Edit tag & note…" menu. Schema is versioned on disk so future multi-tag / per-tag-note shapes can migrate in place.
